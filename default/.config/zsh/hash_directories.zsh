hash -d aps=~/documents/ecole/6/aps
hash -d cpa=~/documents/ecole/6/cpa
hash -d pnl=~/documents/ecole/6/pnl
hash -d pc2r=~/documents/ecole/6/pc2r

hash -d aaga=~/documents/ecole/7/aaga
hash -d dar=~/documents/ecole/7/dar
hash -d tas=~/documents/ecole/7/tas
hash -d svp=~/documents/ecole/7/svp
hash -d ppc=~/documents/ecole/7/ppc
hash -d gral=~/documents/ecole/7/gral

hash -d nvim=~/.config/nvim
hash -d zsh=~/.config/zsh

hash -d cv=~/documents/cv_motiv/src
