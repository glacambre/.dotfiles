...
if 1
    ...
    "Compiles and updates tex files when saving
    function! CompileAndUpdate()
        !pdflatex % && (if [ -e %:r.toc ]; then; pdflatex %; fi; found=false; for i in `pidof mupdf-x11`; do; if [ `xargs -0 < /proc/$i/cmdline | grep -c "%:r.pdf"` -ge 1 ] ; then; kill -1 $i; found=true; fi; done; if [ $found = false ]; then; mupdf %:r.pdf &; fi;)
    endfunction
